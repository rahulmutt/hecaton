...
- **Plugin ↔ daemon** — operator-installed packages running sandboxed; `hello`, the host routes (`fleets`, `actions`, `kv`, `attach`, `fleets/watch`) and, the other way, `activate`/`deactivate`/`events`/`intercept`/`health`/`metrics`/the proxied `/v1/routes/*` at the address the plugin gave in `hello` cross it over loopback HTTP with a per-plugin token. The daemon presents the plugin's own token on every call it makes to the plugin.
